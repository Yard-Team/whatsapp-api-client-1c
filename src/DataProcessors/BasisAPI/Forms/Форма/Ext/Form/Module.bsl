...
Функция ХостВебхуковПоУмолчанию() Экспорт
	Возврат "https://webhook.basis-api.com";
КонецФункции
...
Функция ОткрытьФормуОбработки(ИмяФормы)
	...
	Если ЭтоВнешняяОбработкаОтчет() Тогда
		ИмяОткрФормы = "ВнешняяОбработка.BasisAPI.Форма." + ИмяФормы;
	Иначе
		ИмяОткрФормы = "Обработка.BasisAPI.Форма." + ИмяФормы;
	КонецЕсли;
...
Процедура ПодготовитьПараметрыЗапросов()
	...
	|    ""countryInstance"": ""ru"",
	|    ""webhookUrl"": ""https://mysite.com/webhook/basis-api/"",
	...
	ПараметрИмяГруппы = "Group created by Basis API";
